...
class FlashMHA(nn.Module):
    ...
    def forward(self, x, key_padding_mask=None, cu_seqlens=None, max_seqlen=None, need_weights=False):
        ...
        if self.use_rotary_emb:
            ...
            qkv = rearrange(qkv, 'nnz (three h d) -> nnz three h d', three=3, h=self.num_heads)
        context, attn_weights = self.inner_attn(qkv, key_padding_mask=key_padding_mask,
                                                need_weights=need_weights, causal=self.causal,
                                                cu_seqlens=cu_seqlens, max_s=max_seqlen)
        if qkv.dim() == 3:
            return self.out_proj(rearrange(context, 'b s h d -> b s (h d)')), attn_weights
        else:
            return self.out_proj(rearrange(context, 'nnz h d -> nnz (h d)')), attn_weights